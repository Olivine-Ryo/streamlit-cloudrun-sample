...
def main():
    ...
    accounting_type = st.selectbox(
        "算出手法を選択", df_column_meta["accounting_type"].unique()
    )
    df_column_meta = df_column_meta[
        df_column_meta["accounting_type"] == accounting_type
    ]
    ...
    if (
        ...
        is_relative = st.checkbox(
            "世界全体に対する相対値で表示",
            value=False,
            disabled=((gas_type != "二酸化炭素") | (accounting_type != "生産ベース"))
            & (count_type != "貿易に伴う排出量"),
        )
    ...
    with tab2:
        ...
        fig = px.choropleth(
            ...
            locations="iso_code",
            color=column_name,
            hover_data=["country", column_name],
    ...
    with tab3:
        ...
        year_start = st.slider(
            "比較する年を選択(古)",
            min_value=int(df_plot["year"].min()),
            max_value=int(df_plot["year"].max()) - 1,
            value=int(df_plot["year"].max()) - 10,
            step=1,
        )
        year_end = st.slider(
            "比較する年を選択(新)",
            min_value=int(year_start) + 1,
            max_value=int(df_plot["year"].max()),
            value=int(df_plot["year"].max()),
            step=1,
        )
        df_plot = df[(df["year"].isin([year_start, year_end]))].pivot_table(
            column_name, "country", "year"
        )
        df_plot[f"{year_start}年と{year_end}年の差({unit_name})"] = (
            df_plot[year_end] - df_plot[year_start]
        )
        df_plot[f"{year_start}年と{year_end}年の変化率(%)"] = 100 * (
            df_plot[year_end].div(df_plot[year_start]) - 1.0
        )
